...
$sql = "
    SELECT o.order_id, o.date_added as order_date, op.product_id, op.name, op.model, op.quantity as q_remove, op.price, p.quantity as q_total, pi.image
    FROM " . DB_PREFIX . "order_product op
    LEFT JOIN " . DB_PREFIX . "product p ON p.product_id = op.product_id
    LEFT JOIN " . DB_PREFIX . "product_image pi ON pi.product_id = op.product_id
...
if ($query->num_rows > 0) {
    foreach ($query->rows as $row) {
        ...
        $db->query("INSERT INTO `sale_analytics` (`product_id`, `model`, `name`, `image`, `price`, `q_remove`, `q_total`, `order_id`, `order_date`) VALUES (" . (int)$row['product_id'] . ", '" . $row['model'] . "', '" . $row['name'] . "', '" . $row['image'] . "', '" . $row['price'] . "', '" . $row['q_remove'] . "', '" . $row['q_total'] . "', '" . $row['order_id'] . "', '" . $row['order_date'] . "' );");
    }
